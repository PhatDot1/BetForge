...
async function main() {
  ...
  const contractAddress = '0x9D6E64d6dE2251c1121c1f1f163794EbA5Cf97F1'; // Replace with Optimism Sepolia contract address
  const OptimismNFTMinter = await hre.ethers.getContractFactory('OptimismNFTMinterX');
  const contract = await OptimismNFTMinter.attach(contractAddress);
  ...
  if (mintEvent && mintEvent.args) {
    ...
    // Generate the Optimism Sepolia Etherscan URL for the minted NFT
    const nftUrl = `https://sepolia-optimism.etherscan.io/nft/${contractAddress}/${tokenId}`;
